...
readonly INSTANCE_NAME=youth-api
OPER=${1}
SYS_ID=1001
...
SYS_MAIN_CLASS=com.wu.mall.core.SiteMain
COMMAND=shutdown
...
JAVA_OPTS+=" -Xmx2048m -Xms2048m -XX:NewRatio=2 -XX:MaxMetaspaceSize=512m -XX:-UseBiasedLocking -XX:CompileThreshold=20000"
...
JAVA_OPTS+=" -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5007"

#Custom lib path
export CLASSPATH=$CONF_DIR:$LIB_DIR/*:$SYS_DIR/*
...
start()
{
    ...
    echo "timeout(50s)!!! Please check log: ${INSTANCE_NAME}.out!"
    return 0
}
 ...
 #   nohup $JAVA_HOME/bin/java -cp $CLASSPATH -Dwebank.sys.id=${SYS_ID} -Dwebank.sys.name=${INSTANCE_NAME} -Dindex.log.home=${LOG_DIR} \
  #      ${SYS_MAIN_CLASS} -d ${SYS_HOST} -p ${SYS_PORT} -s ${COMMAND} stop >> ${LOG_DIR}/${INSTANCE_NAME}.out 2>&1 &
   ...
   # while [ $loopcount -lt 10 ];do
    ...
    #echo "timeout(90s)!!! Please check log: ${INSTANCE_NAME}.out!"

    #force quit
    fuser -s -k $JMX_PORT/tcp
    echo "${INSTANCE_NAME} was killed! pid is: `cat $JAVA_PID`"
	kill -9 `cat $JAVA_PID`
